...
def main(args):
    ...
    testset = torchvision.datasets.SVHN('./data/svhn', download=True, split='test',
                                             transform=transforms.Compose([
                                             transforms.Resize(args.image_size),
                                             transforms.ToTensor(),
                                             transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
def test(ebm_net, flow_net, testloader, device, args):
    flow_net.eval()
    ebm_net.eval()
    ori_dir = './exp_svhn/ori_samples'
    gen_dir = './exp_svhn/gen_samples'
    ...
    for x, _ in testloader:
        ...
        for i in range(batch_size):
            ...
            if counter >= total_imgs:
                ...
                torchvision.utils.save_image(images_concat, 'exp_svhn/ebm.png')
                images_concat = torchvision.utils.make_grid(grid_img_flow, nrow = int(len(grid_img) ** 0.5), padding=2, pad_value=255, normalize=True)
                torchvision.utils.save_image(images_concat, 'exp_svhn/flow.png')
                return



if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CoopFlow on SVHN')
    ...
    parser.add_argument('--save_dir', type=str, default='exp_svhn/Coop_samples', help='Directory for saving samples')
    parser.add_argument('--ckpt_dir', type=str, default='exp_svhn/save', help='Directory for saving samples')
