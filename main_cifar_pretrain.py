...
def main(args):
    ...
    if args.resume:
        # Load checkpoint.
        file_name = "ckpt/cifar_pretrain.pth.tar"
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        gnorms_flow, gnorms_ebm = train(epoch, ebm_net, flow_net, trainloader, device, ebm_optimizer, ebm_scheduler, flow_optimizer, flow_scheduler,
              loss_fn, args.max_grad_norm, args, save_checkpoint=(epoch % 10 == 0), gnorms_flow=gnorms_flow, gnorms_ebm=gnorms_ebm)
        if epoch % 1 == 0:
...
def train(epoch, ebm_net, flow_net, trainloader, device, ebm_optimizer, ebm_scheduler, flow_optimizer, flow_scheduler, loss_fn, max_grad_norm, args, save_checkpoint=True, gnorms_flow=None, gnorms_ebm=None):
    ...
    for x, _ in trainloader:
        ...
        if (counter % args.n_update_flow == 0) and epoch > 25:  # only update ebm at the first 10 epochs
        ...
        if ebm_grad_norm < args.skip_threshold_ebm or epoch == 0:
            ...
            if epoch > 25 and counter % args.n_update_flow == 0 and flow_grad_norm < args.skip_threshold_flow: 
                flow_optimizer.step()
...
def test(ebm_net, flow_net, testloader, device, args):
    flow_net.eval()
    ebm_net.eval()
    ori_dir = './exp_cifar_pre/ori_samples'
    gen_dir = './exp_cifar_pre/gen_samples'
    flow_dir = './exp_cifar_pre/flow_samples'
    ...
    for x, _ in testloader:
        ...
        for i in range(batch_size):
            ...
            if counter >= total_imgs:
                ...
                torchvision.utils.save_image(images_concat, 'exp_cifar_pre/ebm.png')
                images_concat = torchvision.utils.make_grid(grid_img_flow, nrow = int(len(grid_img) ** 0.5), padding=2, pad_value=255, normalize=True)
                torchvision.utils.save_image(images_concat, 'exp_cifar_pre/flow.png')
                return
...
def test_in_code(ebm_net, flow_net, args, real_m, real_s):
    ...
    for i in range(200):
        ...
        x_ebm = ebm_sample(0, ebm_net, m=64, n_ch=3, im_w=32, im_h=32, K=args.num_steps_Langevin_coopNet, step_size=0.04, device="cuda:0", p_0=x_flow, num_sample=batch_size, save_images=False)
        x_ebm = (x_ebm + 1.0) * 0.5
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CoopFlow on cifar-10')
    ...
    parser.add_argument('--save_dir', type=str, default='exp_cifar_pre/Coop_samples', help='Directory for saving samples')
    parser.add_argument('--ckpt_dir', type=str, default='exp_cifar_pre/save', help='Directory for saving samples')
    ...
    parser.add_argument('--step_size', default=0.04, type=float, help='Langevin step size') # trained with 0.03
    ...
    parser.add_argument('--skip_threshold_flow', default=5e5, type=float, help='skip gradient update')
    parser.add_argument('--grad_norm_ebm', default=5e5, type=float, help='clip gradient norm')
    parser.add_argument('--skip_threshold_ebm', default=5e5, type=float, help='skip gradient update')
