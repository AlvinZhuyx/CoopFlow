...
def main(args):
    ...
    if args.train:
        file_name = "./flow_ckpt/best.pth.tar"
    ...
    if args.resume:
        # Load checkpoint.
        file_name = "ckpt/celeba_pretrain.pth.tar"
...
def test(ebm_net, flow_net, testloader, device, args):
    flow_net.eval()
    ebm_net.eval()
    ori_dir = './exp_celeba_pre/ori_samples'
    gen_dir = './exp_celeba_pre/gen_samples'
    flow_dir = './exp_celeba_pre/flow_samples'
    ...
    for x, _ in testloader:
        ...
        for i in range(batch_size):
            ...
            if counter >= total_imgs:
                ...
                torchvision.utils.save_image(images_concat, 'exp_celeba_pre/ebm.png')
                images_concat = torchvision.utils.make_grid(grid_img_flow, nrow = int(len(grid_img) ** 0.5), padding=2, pad_value=255, normalize=True)
                torchvision.utils.save_image(images_concat, 'exp_celeba_pre/flow.png')
                return


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CoopFlow(pre) on CelebA')
    ...
    parser.add_argument('--save_dir', type=str, default='exp_celeba_pre/Coop_samples', help='Directory for saving samples')
    parser.add_argument('--ckpt_dir', type=str, default='exp_celeba_pre/save', help='Directory for saving samples')
    ...
    parser.add_argument('--n_update_flow', default=5, type=int, help='number of ebm updates between flow model update')
